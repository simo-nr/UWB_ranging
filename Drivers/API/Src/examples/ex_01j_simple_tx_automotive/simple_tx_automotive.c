/*! ----------------------------------------------------------------------------
 *  @file    simple_tx_automotive.c
 *  @brief   Simple TX + Automotive example code
 *
 * @author Decawave
 *
 ...
 *     - byte 2 -> 9: device ID, see NOTE 3 below.
 */
...
int simple_tx_automotive(void)
{
 ...
 #ifndef AUTO_PLL_CAL
 /* AUTO_PLL_CAL must be defined in deca_device_api.h. */
     while (1) { test_run_info((unsigned char *)"\r\nDefine AUTO_PLL_CAL in deca_device_api.h\r\n"); };
 #endif
    ...
    while (1)
    {
        /* Write frame data to DW IC and prepare transmission. See NOTE 5 below.*/
        ...
        /* Poll DW IC until TX frame sent event set. See NOTE 2 below.
 ...
 * 2. We use polled mode of operation here to keep the example as simple as possible, but the TXFRS and CCA_FAIL status events can be used to generate
